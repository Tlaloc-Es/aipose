...
def xywh2xyxy(x):
    """Convert nx4 boxes from [x, y, w, h] to [x1, y1, x2, y2] where xy1=top-left, xy2=bottom-right"""  # noqa: E501
...
def xyxy2xywh(x):
    """Convert nx4 boxes from [x1, y1, x2, y2] to [x, y, w, h] where xy1=top-left, xy2=bottom-right"""  # noqa: E501
    y = x.clone() if isinstance(x, torch.Tensor) else np.copy(x)
...
def non_max_suppression_kpt(  # noqa: C901
    prediction,
    conf_thres=0.25,
    iou_thres=0.45,
    classes=None,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        # Apply constraints
        # x[((x[..., 2:4] < min_wh) | (x[..., 2:4] > max_wh)).any(1), 4] = 0  # width-height # noqa: E501
